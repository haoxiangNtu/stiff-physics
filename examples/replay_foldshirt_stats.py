...
def main():
    ...
    if int(os.environ.get("CASE39ME_HEADLESS","0")):
        ...
        ms = []
        nw = []
        _n0 = eng.native.get_total_newton_iters()
        for fr in range(f0, f1):
            ...
            _n1 = eng.native.get_total_newton_iters(); nw.append(int(_n1-_n0)); _n0 = _n1
            if fr % 20 == 0:
                v = eng.get_vertices()
        ...
        import numpy as _np
        _nw = _np.array(nw); _pk = int(_nw.argmax())
        print(f"[fs-stats] wall_sum={sum(ms)/1000.0:.1f}s newton_sum={int(_nw.sum())} newton_peak={int(_nw.max())}@fr{_pk} newton_mean={float(_nw.mean()):.2f} step_peak={max(ms):.0f}ms@fr{int(_np.array(ms).argmax())}", flush=True)

#!/usr/bin/env python3
"""Franka Panda + Table + Shirt + trajectory playback — replicates ./gipc 21.

Loads the Franka Panda URDF, a fixed table, and a shirt mesh. After 10
settling frames, joint targets are driven by franka_fold.txt trajectory.

Usage:
    python examples/case_21_franka_table_cloth.py
"""
...
def main():
    ...
    engine.native.load_urdf(
        assets_dir + "sim_data/urdf/franka_panda/panda_arm_hand.urdf",
    ...
    def callback():
        ...
        expanded, _ = psim.Begin("Franka + Table + Shirt (Case 21)", True)
        if expanded:
            if running[0]:

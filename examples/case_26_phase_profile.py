#!/usr/bin/env python3
"""Headless per-phase profiler for case_26 (semi-implicit arm + shirt).

Runs the same physics setup as case_26_arm_cloth_semi_implicit.py without
polyscope, then steps N frames and reports per-phase wall-clock breakdown
(hessian / pcg / ccd / linesearch / postls).

Output: a per-frame CSV-style line plus an aggregate summary at the end.

Usage:
    python examples/case_26_phase_profile.py [warmup=20] [measure=200]
"""
...
def _resolve_assets_dir() -> str:
    ...
    for name in ("assets", "Assets"):
        cand = here / name
        if (cand / "sim_data" / "urdf" / "xarm" / "xarm7_with_gripper.urdf").exists():
            return str(cand) + "/"
    raise FileNotFoundError(f"No assets dir with xarm urdf found under {here}")
...
def main():
    ...
    # Sanity: verify the diag API exists
    ...
    totals_engine = []   # sum reported by engine (per-frame)
    totals_wall = []     # python-side wall clock
    ...
    print()
    print("=== Aggregate (median, mean, fraction of engine total) ===")
